...
5. **底层支撑分层**:
   - **[`services/`](services/README.md)**: 核心跨 Tool 业务流程流转、结果加工过滤与数据库业务编排。
   - **[`adapters/`](adapters/README.md)**: 浏览器实例、第三方搜索源、文件缓存及数据库连接/仓储适配。
   - **[`schemas/`](schemas/README.md)**: 跨层交互的强类型共享数据结构。
...
- **数据库约束**: Tool 不得直接访问 ORM 或 session；数据库相关能力必须经由 `services/` 和 `adapters/` 流转。
